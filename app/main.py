...
DEFAULT_PUBLIC_BASE_URL = "https://1206v2-production.up.railway.app"
...
BASE_CONTEXT_FILES = [
    ...
    "canon/east_sector/east_sector_runtime_context.md",
...
CHARACTER_PATHS = {
    "akira": "characters/akira/akira_main_profile.yaml",
    "akira_akatsumi": "characters/akira/akira_main_profile.yaml",
    "jun_carter": "characters/jun/jun_main_profile.yaml",
    "jun": "characters/jun/jun_main_profile.yaml",
    "irey": "characters/irey/irey_main_profile.yaml",
    "emma": "characters/emma/emma_main_profile.yaml",
    "raiden_sterling": "characters/raiden/raiden_main_profile.yaml",
    "raiden": "characters/raiden/raiden_main_profile.yaml",
    "ray_carter": "characters/ray/ray_main_profile.yaml",
    "ray": "characters/ray/ray_main_profile.yaml",
...
CHARACTER_LINKED_FILES = {
    "akira": ["characters/akira/akira_thought_triggers.yaml"],
    "akira_akatsumi": ["characters/akira/akira_thought_triggers.yaml"],
    "jun_carter": ["characters/jun/jun_knowledge_connections.yaml"],
    "jun": ["characters/jun/jun_knowledge_connections.yaml"],
    "irey": ["characters/irey/irey_knowledge_connections.yaml"],
    "emma": ["characters/emma/emma_knowledge_connections.yaml"],
    "raiden_sterling": ["characters/raiden/raiden_knowledge_connections.yaml"],
    "raiden": ["characters/raiden/raiden_knowledge_connections.yaml"],
    "ray_carter": ["characters/ray/ray_knowledge_connections.yaml"],
    "ray": ["characters/ray/ray_knowledge_connections.yaml"],
}

CONDITIONAL_LORE_FILES = {
    "akira_hidden": "characters/akira/akira_hidden_past.yaml",
    "raiden_hidden": "characters/raiden/raiden_hidden_past.yaml",
    "ray_hidden": "characters/ray/ray_hidden_past.yaml",
    "jun_hidden": "characters/jun/jun_hidden_past.yaml",
    "emma_hidden": "characters/emma/emma_hidden_past.yaml",
    "irey_hidden": "characters/irey/irey_hidden_past.yaml",
    "akira_raiden_bond": "canon/relationships/akira_raiden_hidden_bond.yaml",
    "reincarnation_cycle": "canon/hidden/reincarnation_cycle_hidden_lore.yaml",
    "kairos_echo_background": "canon/kairos_echo_background.md",
    "energy_rules_index": "canon/energy_rules_index.md",
}

KNOWLEDGE_PATHS = {
    "akira": "characters/akira/akira_knowledge_connections.yaml",
    "akira_akatsumi": "characters/akira/akira_knowledge_connections.yaml",
    "jun_carter": "characters/jun/jun_knowledge_connections.yaml",
    "jun": "characters/jun/jun_knowledge_connections.yaml",
    "irey": "characters/irey/irey_knowledge_connections.yaml",
    "emma": "characters/emma/emma_knowledge_connections.yaml",
    ...
    "ray_carter": "characters/ray/ray_knowledge_connections.yaml",
    "ray": "characters/ray/ray_knowledge_connections.yaml",
    "haru_foster": "knowledge/characters/haru_foster_knowledge.md",
    "haru": "knowledge/characters/haru_foster_knowledge.md",
    "yuna": "knowledge/characters/yuna_knowledge.md",
    "miki_larsen": "knowledge/characters/miki_larsen_knowledge.md",
    "miki": "knowledge/characters/miki_larsen_knowledge.md",
}
...
def conditional_lore_for(user_input: str, state: Dict[str, Any]) -> List[str]:
    text = normalized_text(user_input)
    active = {str(x) for x in list(state.get("active_characters") or []) + list(state.get("nearby_characters") or []) + list(state.get("conditional_characters") or [])}
    files: List[str] = []

    def has_any(*words: str) -> bool:
        return any(w in text for w in words)

    if has_any("прошл", "памят", "вспом", "срыв", "шрам", "кольц", "самуэл", "самуэль", "ребен", "ребён", "кайрос", "браслет", "северн", "западн", "эхо"):
        files.append(CONDITIONAL_LORE_FILES["akira_hidden"])
    if ("akira" in active or "akira_akatsumi" in active) and ("raiden_sterling" in active or "raiden" in active or has_any("райден", "рейден", "кольц", "ar", "ар")):
        files.append(CONDITIONAL_LORE_FILES["akira_raiden_bond"])
    if has_any("реинкар", "цикл", "наблюд", "эйрен", "прошлые жизни", "прошлая жизнь", "повтор"):
        files.append(CONDITIONAL_LORE_FILES["reincarnation_cycle"])
    if has_any("эхо", "кайрос", "каэрис", "каирос"):
        files.append(CONDITIONAL_LORE_FILES["kairos_echo_background"])
    if has_any("энерг", "поток", "перегруз", "контрол", "простран"):
        files.append(CONDITIONAL_LORE_FILES["energy_rules_index"])
    for key, char_id in [("raiden_hidden", "raiden_sterling"), ("ray_hidden", "ray_carter"), ("jun_hidden", "jun_carter"), ("emma_hidden", "emma"), ("irey_hidden", "irey")]:
        if char_id in active and has_any("прошл", "памят", "знает", "скрыт", "связ", "самуэл", "самуэль"):
            files.append(CONDITIONAL_LORE_FILES[key])
    return files


def build_required_files(session_id: str, user_input: str = "", mode: str = "play") -> List[str]:
    ensure_session_state(session_id)
    state = current_state(session_id)
    resolved_mode = classify_mode(user_input, mode)
    files = build_file_list(state, resolved_mode, session_id, user_input)
    active = list(state.get("active_characters") or [])
    nearby = list(state.get("nearby_characters") or state.get("nearby_or_possible") or [])
    conditional = list(state.get("conditional_characters") or [])
    for character_id in list(dict.fromkeys(active + nearby + conditional)):
        for linked in CHARACTER_LINKED_FILES.get(str(character_id), []):
            files.append(linked)
    files.extend(conditional_lore_for(user_input, state))
    return [path for path in dict.fromkeys(files) if file_exists_for_context(path, session_id)]
...
def build_turn_contract(session_id: str, req: TurnContractRequest) -> Dict[str, Any]:
    ...
    files = build_required_files(session_id, req.user_input, mode)
...
def actions_schema_json() -> Dict[str, Any]:
    return {
        "openapi": "3.1.0",
        "info": {"title": "Akira Main 1206 API", "version": "1.6.0"},
        ...
        "paths": {
            ...
            "/api/v1/sessions/{session_id}/required-files/manifest": {"get": {"operationId": "getRequiredFilesManifest", "summary": "Get ordered required file manifest", "parameters": [session_parameter(), {"name": "user_input", "in": "query", "required": False, "schema": {"type": "string"}}, {"name": "mode", "in": "query", "required": False, "schema": {"type": "string", "enum": ["play", "technical", "audit", "transfer"], "default": "play"}}], "responses": {"200": {"description": "Required files manifest"}}}},
            "/api/v1/sessions/{session_id}/required-files/chunk": {"get": {"operationId": "getRequiredFilesChunk", "summary": "Read required files by chunks", "parameters": [session_parameter(), {"name": "chunk_index", "in": "query", "required": False, "schema": {"type": "integer", "default": 0}}, {"name": "max_chars", "in": "query", "required": False, "schema": {"type": "integer", "default": 60000}}, {"name": "max_items", "in": "query", "required": False, "schema": {"type": "integer", "default": 6}}, {"name": "user_input", "in": "query", "required": False, "schema": {"type": "string"}}, {"name": "mode", "in": "query", "required": False, "schema": {"type": "string", "enum": ["play", "technical", "audit", "transfer"], "default": "play"}}], "responses": {"200": {"description": "Required files chunk"}}}},
...
def root() -> Dict[str, Any]:
    return {"status": "ok", "service": "Akira Main 1206 API", "version": "1.6.0", "actions_schema_json": "/openapi-actions.json"}
...
def required_files_manifest(session_id: str, user_input: str = "", mode: str = "play") -> Dict[str, Any]:
    sid = safe_session_id(session_id)
    files = build_required_files(sid, user_input, mode)
    return {
        "success": True,
        "session_id": sid,
        "mode": classify_mode(user_input, mode),
        "required_file_count": len(files),
        "required_files": files,
        "chunk_endpoint": f"/api/v1/sessions/{sid}/required-files/chunk",
        "usage": "Call getRequiredFilesChunk with chunk_index=0 and repeat next_chunk_index until has_more=false.",
    }


@app.get("/api/v1/sessions/{session_id}/required-files/chunk")
def required_files_chunk(session_id: str, chunk_index: int = 0, max_chars: int = 60000, max_items: int = 6, user_input: str = "", mode: str = "play") -> Dict[str, Any]:
    sid = safe_session_id(session_id)
    files = build_required_files(sid, user_input, mode)
    start = max(0, int(chunk_index))
    max_items = max(1, min(int(max_items), 20))
    max_chars = max(1000, min(int(max_chars), 200000))
    items = []
    used_chars = 0
    idx = start
    while idx < len(files) and len(items) < max_items:
        path = files[idx]
        text = contract_file_text(path, current_state(sid), sid)
        remaining = max_chars - used_chars
        if remaining <= 0:
            break
        trimmed = trim_text(text, remaining)
        items.append({"index": idx, "path": path, **trimmed})
        used_chars += len(trimmed.get("content") or "")
        idx += 1
    return {
        "success": True,
        "session_id": sid,
        "chunk_index": start,
        "next_chunk_index": idx if idx < len(files) else None,
        "has_more": idx < len(files),
        "required_file_count": len(files),
