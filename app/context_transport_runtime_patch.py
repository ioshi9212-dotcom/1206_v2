...
Current visible scene format is the selected visual-novel header:
...
## Medium scene style digest — current visual-novel format

Use the selected selected visual-novel header/footer format.
...
ID_ALIASES = {
    ...
    "char_jun": "jun",
    "jun_carter": "jun",
    # Add aliases for new 1206 characters here.
}
...
def medium_output_format_contract() -> dict[str, Any]:
    ...
    required_rules = [
        "Scene MUST start with the selected visual-novel header: 🌘 Астрейн · date, then 🕒 time · 📍 current location from state, 🌦️ weather, ⚙️ active scene state.",
    ...
    original["scene_header_template"] = {
        "required": True,
        "style": "1206_visual_novel_header_v2",
        "example": [
            "🌘 Астрейн · 1206 г., 31 августа, день уточнить",
            "🕒 02:40 · 📍 Дом Джуна",
            ...
            "⚙️ Активное состояние сцены: учитывать в тексте, действиях и предметах",
            "",
            "✦ Настороженная · после ночного старта · волосы как указано в state",
        ...
        "forbidden": [
            ...
            "plain raw field list without the selected header",
...
def build_scene_context_digest(session_id: str) -> str:
    ...
    rule_digest = {
        "output": [
            ...
            "Scene must start with selected visual-novel header, not loose 🗓️ card header.",
...
def get_required_files_chunk_safe(
    ...
    sid = base.safe_session_id(session_id)
    return ccp.RequiredFilesChunkResponse(session_id=sid, required_files=[], chunk_index=0, chunks_total=0, has_more=False, next_chunk_index=None, loaded_files=[], missing_files=[], loaded_count=0, missing_count=0, total_loaded_parts=0)
...
def get_required_files_bundle_safe(
    ...
    return get_required_files_chunk_safe(session_id, chunk_index=chunk_index, max_chars=max_chars, max_items=max_items)
