...
The visible scene format reminder now matches the current selected header.
"""
...
def build_scene_context_digest(session_id: str) -> str:
    ...
    rule_digest = {
        "output": [
            ...
            "Scene must start with the current selected visual-novel header, not the obsolete loose 🗓️ card header.",
            "Header: 🌘 Астрейн · date, then 🕒 time · 📍 current location from state, 🌦️ weather, ⚙️ active scene state, then ✦/🧥/◈ lines and separator.",
