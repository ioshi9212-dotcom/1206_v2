...
APP_NAME = "Akira 1206 v2 API"
BASE_URL = os.getenv("PUBLIC_BASE_URL", "https://1206v2-production.up.railway.app").rstrip("/")
ROOT = Path(__file__).resolve().parents[1]
DATA = Path(os.getenv("DATA_DIR", "/data"))

SYNC_FROM_REPO = ["calendar", "canon_lore", "characters", "gpt", "templates", "engine", "validation"]
STATE_SEED = ["state"]
...
def repair_state(session_id: str | None = None) -> RepairResponse:
    ...
    if "akira" not in active:
        active.append("akira")
    if "jun" not in active and "jun" not in nearby:
        nearby.append("jun")
        notes.append("Added jun as nearby at 1206 v2 start.")
    current.setdefault("visible_inventory", [])
    current.setdefault("nearby_items", [])
    current.setdefault("current_scene_goal", "старт 1206 v2 в доме Джуна")
...
def session_turn_contract(session_id: str):
    ...
    return {
        ...
        "forbidden_new_facts_this_turn": [
            ...
            "future events as current facts",
            "hidden facts revealed without scene basis",
